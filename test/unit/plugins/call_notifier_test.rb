...
class CallNotifierTest < Test::Unit::TestCase
  include FileSandbox
  ...
  def setup
    ...
    @notifier = CallNotifier.new
    @notifier.numbers = ["012"+"345"+"6789", "012"+"345"+"6780"]
    @notifier.from = "012"+"345"+"6789"    
    
    @project.add_plugin(@notifier, :test_call_notifier)
  end
  ...
  def test_send_email_with_failing_build
    ...
    assert_equal @notifier.numbers, mail.to
  ...
  def test_logging_on_send
    ...
    @notifier.numbers = ['foo@happy.com', 'bar@feet.com', 'you@me.com', 'uncle@tom.com']
    @notifier.build_finished(failing_build)

    CruiseControl::Log.expects(:event).with("Sent e-mail to 1 person", :debug)
    BuildMailer.expects(:deliver_build_report)
    @notifier.numbers = ['foo@happy.com']
    @notifier.build_finished(failing_build)
  ...
  def test_useful_errors
    ...
    @notifier.numbers = ['foo@crapty.com']
